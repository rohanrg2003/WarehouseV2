...
# Always use /tmp — works on Vercel AND locally during dev
# On local dev this just means /tmp/warehouse.db instead of ./warehouse.db
# If you want local persistence, change to "warehouse.db"
# /tmp works on Vercel/Linux — on Windows dev use local file
DB_PATH = os.environ.get("DB_PATH", "/tmp/warehouse.db" if os.name != "nt" else "warehouse.db")
...
def init_db():
    ...
    try:
        ...
        """)
        # seed admin — fails silently if already exists
        try:
            con.execute(
...
# Safe because all tables use IF NOT EXISTS
init_db()
...
def seller():
    ...
    tx_count = query("SELECT COUNT(*) AS c FROM transactions WHERE seller_id=?", (sid,), one=True)["c"]
...
def edit_product(pid):
    ...
    cat_ids = request.form.getlist("cat_ids")
    try:
        run("UPDATE products SET name=?, sku=?, price=?, quantity=?, expiry=? WHERE id=? AND seller_id=?",
...
def delete_product(pid):
    ...
    if count > 0:
        flash(f"Cannot delete — {count} transaction(s) linked.", "error")
    else:
...
def create_transaction():
    ...
    if not p:
        ...
        flash(f'Only {p["quantity"]} units of "{p["name"]}" left!', "error")
    else:
        total = round(p["price"] * qty, 2)
        run("UPDATE products SET quantity = quantity - ? WHERE id=?", (qty, pid))
        run("INSERT INTO transactions (seller_id, product_id, quantity, price, total_price, type) VALUES (?,?,?,?,?,'Sale')",
